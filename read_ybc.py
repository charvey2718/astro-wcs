...
def readYBC(filename='bsc5.dat', path=None):
	...
	# data = pd.read_fwf(file, names=names, colspecs=fix_colspecs) # Doesn't work with pandas 0.25.3
	data = pd.read_fwf(filename, colspecs=fix_colspecs, header=None) # So just read it in without names...
	data.columns = names # ...and add the column names afterwards
