...
if __name__ == "__main__":
	...
	for fits_file in fits_files: # loop through all fits files in directory
		...
		for alt in range(15, 166, 15):
			...
			alt_vertices = Polygon(np.vstack([x, y]).T, False, edgecolor='blue', facecolor='none', lw = 1, ls='dotted', alpha=0.75)
		...
		for az in range(0, 361, 60):
			...
			x, y = wcs.all_world2pix(az, alt, 0.)
			az_vertices = Polygon(np.vstack([x, y]).T, False, edgecolor='blue', facecolor='none', lw = 1, ls='dotted', alpha=0.75)
		...
		for dec in range(15, 166, 15):
			...
			x, y = wcs.all_world2pix(az, alt, 0.)
			dec_vertices = Polygon(np.vstack([x, y]).T, False, edgecolor='black', facecolor='none', lw = 1, ls='dotted', alpha=0.75)
		...
		for ra in range(0, 361, 60):
			...
			alt, az = radec2altaz(ra, dec, mjd, location=location)
			x, y = wcs.all_world2pix(az, alt, 0.)
			ra_vertices = Polygon(np.vstack([x, y]).T, False, edgecolor='black', facecolor='none', lw = 1, ls='dotted', alpha=0.75)
