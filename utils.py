...
__all__ = ['lsst_earth_location', 'robustRMS', 'radec2altaz', 'altaz2radec']
...
def altaz2radec(alt, az, mjd, location=None):
    """I need a stupid converter
    """
    if location is None:
        location = lsst_earth_location()
    time = Time(mjd, format='mjd')
    AltAzCoord = SkyCoord(alt=alt*u.degree, az=az*u.degree, obstime=time, frame='altaz', location=location)
    return AltAzCoord.icrs.ra.value, AltAzCoord.icrs.dec.value
